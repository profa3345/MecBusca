...
// ── Ana IA — HTTP endpoint para o frontend (/api/ana) ─────────────────────────
// Wrapper onRequest que reutiliza anthropicKey, checkRateLimit e sanitizeStr
// já definidos acima. O onRequest é necessário porque o frontend chama
// fetch('/api/ana') diretamente — onCall exige o SDK do Firebase no cliente.
// REGION alterado para us-central1 porque southamerica-east1 não suporta
// secrets em onRequest (limitação atual do Firebase Functions v2).
...
exports.anaIA = onRequest(
  ...
  async (req, res) => {
    ...
    // Rate limit — reutiliza a mesma função do resto do arquivo
    const ip = req.headers['x-forwarded-for']?.split(',')[0]?.trim() || req.ip || 'unknown';
    try {
      const ok = await checkRateLimit(ip, 'anaIA');
      if (!ok) return res.status(429).json({ error: 'Muitas mensagens. Aguarde um momento.' });
    } catch (e) {
      console.warn('[anaIA] rate limit falhou:', e.message);
    }
    ...
    // Montar messages — reutiliza sanitizeStr
    const messages = [
      ...Array.isArray(history)
        ? history.slice(-10)
            .filter(m => ['user','assistant'].includes(m?.role) && typeof m?.content === 'string')
            .map(m => ({ role: m.role, content: sanitizeStr(m.content, 2000) }))
        : [],
      { role: 'user', content: sanitizeStr(message, 4000) },
    ];
    ...
    try {
      ...
      console.error('[anaIA] fetch error:', e.message);
      return res.status(502).json({ error: 'Erro de conexão. Tente novamente.' });
    }

    if (!anthropicRes.ok) {
      const status = anthropicRes.status;
      console.error('[anaIA] Anthropic error:', status);
    ...
    // Auditoria de uso — fire-and-forget
    db.collection('_ana_usage').add({
      ip,
      inputTokens: data.usage?.input_tokens || 0,
      outputTokens: data.usage?.output_tokens || 0,
